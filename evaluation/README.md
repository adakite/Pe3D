...
This directory contains validation and benchmark outputs for the Python `pe3d` fork against archived reference outputs from the original workflow and against manually labeled datasets.

The most useful subdirectories for a public repository are:

- `pe3d_current_compare_normalized/`: current reference-vs-`pe3d` Otira comparison after normalizing angle conventions to `[0, 180)`
...
- Orientation remains the main area where the Python fork is less faithful to the reference outputs.
